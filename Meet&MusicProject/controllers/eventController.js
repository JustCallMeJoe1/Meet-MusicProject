...
exports.getSpecificEvent = (req, res, next) => {
    ...
    eventModel.findById(chosenId).then(chosenEvent => {
        ...
        if (chosenEvent) {
            ...
            let formattedMonth = convertNumberToMonth(monthNumber);

            let dayNumber = chosenEvent.date.substring(8,10);

            let dayEnding = convertNumberToPrefix(dayNumber);
            ...
            let formattedStartHour = convertTime(startNumberHour);
            let formattedEndHour = convertTime(endNumberHour);

            let startTimePeriod = convertPeriod(startNumberHour);
            let endTimePeriod = convertPeriod(endNumberHour);
...
let convertNumberToMonth = (monthNumber) => {
    let formattedMonth = "";
    switch(monthNumber) {
        case "01":
...
let convertNumberToPrefix = (dayNumber) => {
    let dayEnding = "";
        switch(dayNumber) {
            case "01":
                dayEnding = "st";
